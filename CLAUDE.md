...
**Scan screen input tabs:** All · Unassigned · Clustered · Scanner Suggestions (4th tab). The Scanner Suggestions tab shows scanner-promoted candidates scoped to this project (`project_id === null`, `is_seeded`, `metadata.source === "scanner"`, `!metadata.dismissed`, with a `metadata.suggested_projects` entry matching `activeProjectId`). Partitioned into Emerging (novel — `metadata.suggested_projects[].classification === "emerging"`) and Reinforcing (confirms existing clusters). Per-row actions (persistently visible in their own fixed-width column — `COL_AI.actions`, not hover-revealed): **Accept** (`saveInputToProject(id, activeProjectId)`) and **Dismiss** (`dismissSuggestedInput(input)`). Batch actions in sticky bar: **Accept N** (`saveInputsToProject`) and **Dismiss N**. Accepted inputs drop from Scanner Suggestions and appear in All/Unassigned immediately via optimistic state. Horizon filter is omitted from this tab — scanner candidates don't have `horizon` set at promotion time.
...
- **`api/clone-sample-project.js`** — the onboarding-facing endpoint. Bearer-auth (same pattern as `seed-onboarding.js`): verifies the token, derives `destWorkspaceId` from the caller's own session server-side, and reads the source project id only from `SAMPLE_TEMPLATE_PROJECT_ID` (env var) — never from the request body. Wired into `App.jsx`'s `handleOnboardingComplete` as a fire-and-forget call alongside the existing `onboarding_completed` write; on success it calls **`appState.refreshAll()`** (NOT `refreshProjects()`) so the clone appears without a manual reload (the redirect itself never waits on it). This must refresh *every* workspace-scoped entity array, not just `projects`: the Dashboard card derives its input/cluster counts from the workspace-scoped `inputs`/`clusters` arrays fetched once at mount, so refreshing only `projects` surfaced the clone as a permanent `0/0` empty card until a full reload. `handleOnboardingComplete` also flips `appState.sampleCloneInProgress` (set when the POST fires, cleared in `.finally()` after `refreshAll()`) to drive an optimistic "Setting up your sample project…" placeholder card/row on the Dashboard during the ~1–3s clone. **Any future clone/duplicate path that creates rows out-of-band must call `refreshAll()`, not per-entity refreshers** — same stale-child trap.
...
### Migrations applied via Dashboard SQL Editor instead of `db push`
On 2026-08-20, `supabase db push`/`supabase link` against staging repeatedly failed SASL auth (`password authentication failed for user "postgres"`, SQLSTATE 28P01) despite multiple DB password resets — root cause never conclusively identified (tried both the direct host and the pooler host, `--db-url` and `SUPABASE_DB_PASSWORD`, quoted and unquoted). Worked around it by running the three `system_map_templates`/`project_system_map_background`/Storage-bucket migration files' contents directly in the Dashboard SQL Editor instead, then confirming the result matched exactly via the `mcp__claude_ai_Supabase__execute_sql` tool (table rows, RLS, bucket). This is **not** the same failure mode as the "Undocumented-schema-change pattern" above — the migration files exist, are correctly timestamped, and are checked into `supabase/migrations/` — only the CLI's own `supabase_migrations.schema_migrations` ledger doesn't know they were applied. A future `supabase db push` once CLI auth works again may report these as pending; since the SQL is idempotent (`if not exists`, `on conflict`) a re-run is harmless, but running `supabase migration repair --status applied 20260820120000 20260820120001 20260820120002` first would reconcile the ledger cleanly instead.
