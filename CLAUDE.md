...
| `docs/system-map-background-templates-spec.md` | Any work touching System Map background templates — `system_map_templates`, `project_system_map_background`, the `system-map-templates` Storage bucket, or the canvas's `BackgroundTemplateNode`. In progress as of 2026-08-21: Pass 2 (canvas rendering + PNG export + curated seed data) shipped; Web Publish integration (Pass 3) and the user-upload stretch goal are not yet built |
